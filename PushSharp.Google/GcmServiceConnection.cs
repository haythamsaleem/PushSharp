...
namespace PushSharp.Google
{
    ...
    public class GcmServiceConnection : IServiceConnection<GcmNotification>
    {
        public GcmServiceConnection(GcmConfiguration configuration)
        {
            ...
            {
                http = getClientViaHTTPProxy(Configuration.ProxyHost, Configuration.ProxyPort);
            }
            else
            {
                http = new HttpClient();
        ...
        private HttpClient getClientViaHTTPProxy(string httpProxyHost, int httpProxyPort)
        {
            HttpClient client = null;
            try
            {
                string proxyUri =
                    string.Format("{0}:{1}", httpProxyHost, httpProxyPort);
